...
size_t map_str_size(map_str *map) { return map->size; }
...
size_t map_str_find(map_str *map, char *key) {
  size_t mask = map->capacity - 1, h = cfarmhash(key, strlen(key)),
         i = h & mask, step = 0;

  while (1) {
    if (map->keys[i] == MAP_STR_RESERVED_EMPTY) return map_str_end(map);

    if (map->keys[i] != MAP_STR_RESERVED_DELETED &&
        strcmp(map->keys[i], key) == 0)
      return i;
...
void map_str_clear(map_str *map) {
  ...
  if (map->release)
    for (position = map_str_begin(map); position != map_str_end(map);
...
size_t map_str_bucket_count(map_str *map) { return map->capacity; }
  ...
  if (map_str_roundup_size(capacity_requested + map->deleted) > map->capacity) {
    ...
    if (map_old.size)
      for (position = map_str_begin(&map_old);
        ...
        map_str_insert(map, map_old.keys[position],
                       map_str_get(&map_old, position));

    if (map_old.keys) free(map_old.keys);

    if (map_old.values) free(map_old.values);
  }
}
...
size_t map_str_begin(map_str *map) { return map_str_next_inclusive(map, 0); }

size_t map_str_next(map_str *map, size_t position) {
  return map_str_next_inclusive(map, position + 1);
}

size_t map_str_end(map_str *map) { return map->capacity; }
...
size_t map_str_find_free(map_str *map, char *key) {
  ...
  while (1) {
    if (map->keys[i] <= MAP_STR_RESERVED_DELETED) return i;

    if (strcmp(map->keys[i], key) == 0) return map_str_end(map);
...
size_t map_str_next_inclusive(map_str *map, size_t position) {
  while (position < map->capacity) {
    if (map->keys[position] > MAP_STR_RESERVED_DELETED) break;
    position++;
  }
...
// For maps that store the full key-value data item as the value,
// we set the internal keys array at the given position to point
// to this address.
